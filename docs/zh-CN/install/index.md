...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=1 npm install -g openclawmu@latest
```
...
pnpm add -g openclawmu@latest           # 重新运行以执行 postinstall 脚本
```
...
- `npm`（默认）：`npm install -g openclawmu@latest`
